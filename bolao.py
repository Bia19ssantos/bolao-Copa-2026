...
# Fuso horário do Brasil para comparação precisa de datas
fuso_br = pytz.timezone("America/Sao_Paulo")
agora_br = datetime.now(fuso_br)
hoje_str = agora_br.strftime("%Y-%m-%d")

# Injeção de CSS para Tema Escuro Perfeito
st.markdown("""
    <style>
        ...
        h1, h2, h3, h4, h5, h6, span, p, label, .stMarkdown { color: #ffffff !important; font-family: 'Arial', sans-serif; }
        ...
        div[data-baseweb="select"], div[data-baseweb="input"], input, .stNumberInput div { background-color: #1f232a !important; color: #ffffff !important; border: 1px solid #3f4756 !important; border-radius: 8px; }
...
# 2. FUNÇÃO PARA LER OS JOGOS FILTRADOS POR DATA
def carregar_jogos():
    try:
        ...
        for _, row in df.iterrows():
            ...
            # Lendo a data da planilha
            data_hora_jogo = datetime.strptime(str(row['Data']).strip(), "%Y-%m-%d %H:%M")
            data_hora_jogo = fuso_br.localize(data_hora_jogo)
...
# Palpites salvos em memória temporária
if "palpites" not in st.session_state:
    st.session_state.palpites = [
        ...
        {"Participante": "Thiago", "Jogo": "Jogo 3", "Palpite": "0 X 1"},
...
with tab1:
    ...
    # FILTRO MÁGICO: Mostra apenas os jogos onde o dia coincide com HOJE na planilha
    jogos_de_hoje = {k: v for k, v in st.session_state.jogos.items() if v['dia'] == hoje_str}
    
    if not jogos_de_hoje:
        st.info("📆 Não existem partidas agendadas para o dia de hoje na planilha!")
    else:
        ...
        if nome != "Selecione seu nome...":
            ...
            # TRAVA DE SEGURANÇA: Bloqueia se o horário do jogo já passou
            if agora_br > dados_jogo["data_completa"] or dados_jogo["encerrado"]:
                st.error("❌ Bloqueado! A partida já começou ou foi encerrada. Não é possível palpitar.")
            else:
